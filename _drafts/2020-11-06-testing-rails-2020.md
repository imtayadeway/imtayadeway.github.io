...
The Mystery Guest is an XUnit antipattern, and the use of factories allows us to follow a better XUnit pattern, namely the [Four-Phase Test][]. The idea behind this pattern is that every test should tell a story:
...
Further, if you are not tied to using Factory Bot, consider using Rails' fixtures instead. If you are already writing idiomatic RSpec there is a good chance that you won't feel any new Mystery Guest pain just by doing this. And your test suite will be faster.
